...
# GAN-Enhanced Steganography Model
class GANSteganographyModel(nn.Module):
    """
    A GAN-enhanced steganography model that embeds secret information in the cover image
    and can extract it from the 3-channel stego image ALONE (no cover needed).
    Includes a discriminator to improve imperceptibility.
    """
    def __init__(self):
        super(GANSteganographyModel, self).__init__()
...
# Discriminator Network - distinguishes between cover and stego images
class Discriminator(nn.Module):
    """
    A discriminator network that distinguishes between cover and stego images
    to improve the imperceptibility of generated stego images.
    """
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            # Block 5
            nn.Conv2d(512, 512, kernel_size=4, stride=2, padding=1),  # 14x14 -> 7x7
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
        )
...
def gan_steganography_loss(cover, stego, secret, recovered_secret, discriminator, 
                          alpha_hid=1.0, alpha_rec=1.0, alpha_adv=0.02):
    """
    GAN-enhanced loss function for steganography:
...
def train_gan_steganography_model(generator, discriminator, dataset, num_epochs=100, 
    ...
    """
    print(f"\nTraining GAN-Enhanced Steganography model with enhanced logging...")
    ...
    print(f"Training completed for GAN-Enhanced Steganography model!")
...
def evaluate_model(generator, test_dataset, num_samples=10):
    """
    Comprehensive evaluation of the trained GAN-enhanced model.
    """
    print(f"\nEvaluating GAN-Enhanced model on {num_samples} test samples...")
...
def create_visualization_grid(results_data, save_path='gan_steganography_results.png'):
    """
    Create a comprehensive visualization grid showing results.
    """
    print(f"\nCreating GAN visualization grid at: {save_path}")
    ...
    plt.close()
    print(f"GAN visualization grid saved at: {save_path}")


def main(config_path="config.json"):
    """
    Main function using config file for GAN-enhanced steganography.
    """
    print("GAN-Enhanced Steganography Training: Embed Secret in Cover, Extract from Stego")
    print("=" * 90)
    ...
    # Initialize models
    gan_steg_model = GANSteganographyModel()
    discriminator = Discriminator()
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    gan_steg_model = gan_steg_model.to(device)
    discriminator = discriminator.to(device)

    print(f"\nGAN-Enhanced Steganography Model parameters: {sum(p.numel() for p in gan_steg_model.parameters()):,}")
    print(f"Discriminator parameters: {sum(p.numel() for p in discriminator.parameters()):,}")
    ...
    # Train GAN-Enhanced Steganography model with enhanced logging
    print("\nStarting GAN-Enhanced Steganography model training (224x224) with enhanced logging...")
    trained_generator, trained_discriminator = train_gan_steganography_model(
        gan_steg_model,
        discriminator,
        train_dataset,
    ...
    print("\n" + "=" * 60)
    print("TRAINING COMPLETED FOR GAN-ENHANCED STEGANOGRAPHY MODEL")
    print("=" * 60)
    ...
    create_visualization_grid(test_results_data, save_path='gan_steganography_test_results.png')

    # Save the trained models
    torch.save({
        'generator_state_dict': trained_generator.state_dict(),
        'discriminator_state_dict': trained_discriminator.state_dict(),
        'model_type': 'GANSteganographyModel',
        'img_size': img_size,
    }, 'gan_steganography_model_50k.pth')

    print(f"\nGAN-Enhanced Steganography model saved as 'gan_steganography_model_50k.pth'")
    ...
    if len(test_dataset) > 0:
        ...
        # Test discriminator
        with torch.no_grad():
            disc_on_cover = discriminator(cover_tensor).mean().item()
            disc_on_stego = discriminator(stego_image).mean().item()
