...
class ImprovedSteganographyModel(nn.Module):
    """
    An improved steganography model that embeds secret information in the cover image
    and can extract it from the 3-channel stego image ALONE (no cover needed).
    """
    def __init__(self):
        super(ImprovedSteganographyModel, self).__init__()
...
def improved_steganography_loss(cover, stego, secret, recovered_secret, alpha_hid=1.0, alpha_rec=1.0):
    """
    Improved loss function for steganography:
    - More balanced weights to improve both imperceptibility and recoverability
    ...
    # Balanced loss - using equal weights (or more balanced than before)
...
def train_steganography_model(model, dataset, num_epochs=100, val_dataset=None, log_dir="runs/steganography",
    ...
    """
    print(f"\nTraining Improved Steganography model with enhanced logging for large dataset...")
    ...
    if config:
        ...
        learning_rate = 1e-4
        alpha_hid = 1.0  # More balanced weight
        alpha_rec = 1.0  # More balanced weight
    ...
    print(f"Loss weights: alpha_hid={alpha_hid}, alpha_rec={alpha_rec}")
    ...
    for epoch in range(num_epochs):
        ...
        for cover_tensor, secret_tensor in progress_bar: # Iterate directly over batches from dataloader
            ...
            try:
                ...
                # Compute loss
                loss, hiding_loss, recovery_loss = improved_steganography_loss(
            ...
            with torch.no_grad(): # Disable gradient computation for validation
                ...
                for val_cover_tensor, val_secret_tensor in val_progress_bar:
                    ...
                    try:
                        ...
                        val_loss, val_hiding_loss, val_recovery_loss = improved_steganography_loss(
...
def main(config_path="config.json"):
    ...
    """
    print("Improved Steganography Training: Embed Secret in Cover, Extract from Stego (with Config)")
    print("=" * 90)
    ...
    # Initialize model
    steg_model = ImprovedSteganographyModel()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    print("\nStarting Improved Steganography model training (224x224) with enhanced logging...")
    ...
    torch.save({
        ...
        'model_type': 'ImprovedSteganographyModel',
        'img_size': img_size,
    }, 'improved_steganography_model_50k.pth')

    print(f"\nImproved Steganography model saved as 'improved_steganography_model_50k.pth'")
