...
class AttentionModule(nn.Module):
    """
    Self-attention module to help the extraction network focus on relevant features.
    """
...
# Enhanced GAN-Enhanced Steganography Model with Attention and Skip Connections
class EnhancedGANSteganographyModel(nn.Module):
    """
    An enhanced GAN-enhanced steganography model with attention mechanisms and skip connections
    ...
    for GAN training stability and improved extraction capabilities.
    """
    def __init__(self):
        super(EnhancedGANSteganographyModel, self).__init__()

        # Enhanced Embedding network - combines cover and secret to produce stego
        self.embed_conv1 = nn.Conv2d(6, 64, kernel_size=3, padding=1)  # 6 = 3 (cover) + 3 (secret)
        self.embed_bn1 = nn.BatchNorm2d(64)
        self.embed_relu1 = nn.ReLU(inplace=True)
        
        self.embed_conv2 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.embed_bn2 = nn.BatchNorm2d(128)
        self.embed_relu2 = nn.ReLU(inplace=True)
        
        self.embed_conv3 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.embed_bn3 = nn.BatchNorm2d(256)
        self.embed_relu3 = nn.ReLU(inplace=True)
        
        self.embed_conv4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.embed_bn4 = nn.BatchNorm2d(256)
        self.embed_relu4 = nn.ReLU(inplace=True)
        
        self.embed_conv5 = nn.Conv2d(256, 128, kernel_size=3, padding=1)
        self.embed_bn5 = nn.BatchNorm2d(128)
        self.embed_relu5 = nn.ReLU(inplace=True)
        
        self.embed_conv6 = nn.Conv2d(128, 64, kernel_size=3, padding=1)
        self.embed_bn6 = nn.BatchNorm2d(64)
        self.embed_relu6 = nn.ReLU(inplace=True)
        
        self.embed_conv7 = nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.embed_bn7 = nn.BatchNorm2d(32)
        self.embed_relu7 = nn.ReLU(inplace=True)
        
        self.embed_conv8 = nn.Conv2d(32, 3, kernel_size=3, padding=1)  # Output 3-channel stego
        self.embed_tanh = nn.Tanh()  # Keep in [-1, 1] range

        # Enhanced Extraction network - extracts secret from stego image ONLY (no cover needed)
        self.extract_conv1 = nn.Conv2d(3, 64, kernel_size=3, padding=1)  # 3 = stego image only
        self.extract_bn1 = nn.BatchNorm2d(64)
        self.extract_relu1 = nn.ReLU(inplace=True)
        
        self.extract_conv2 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.extract_bn2 = nn.BatchNorm2d(128)
        self.extract_relu2 = nn.ReLU(inplace=True)
        
        self.extract_attention1 = AttentionModule(128)  # Attention module
        
        self.extract_conv3 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.extract_bn3 = nn.BatchNorm2d(256)
        self.extract_relu3 = nn.ReLU(inplace=True)
        
        self.extract_attention2 = AttentionModule(256)  # Attention module
        
        self.extract_conv4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.extract_bn4 = nn.BatchNorm2d(256)
        self.extract_relu4 = nn.ReLU(inplace=True)
        
        self.extract_conv5 = nn.Conv2d(256, 128, kernel_size=3, padding=1)
        self.extract_bn5 = nn.BatchNorm2d(128)
        self.extract_relu5 = nn.ReLU(inplace=True)
        
        self.extract_attention3 = AttentionModule(128)  # Attention module
        
        self.extract_conv6 = nn.Conv2d(128, 64, kernel_size=3, padding=1)
        self.extract_bn6 = nn.BatchNorm2d(64)
        self.extract_relu6 = nn.ReLU(inplace=True)
        
        self.extract_conv7 = nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.extract_bn7 = nn.BatchNorm2d(32)
        self.extract_relu7 = nn.ReLU(inplace=True)
        
        self.extract_conv8 = nn.Conv2d(32, 3, kernel_size=3, padding=1)  # Output 3-channel secret
        self.extract_tanh = nn.Tanh()  # Keep in [-1, 1] range
    ...
    def embed(self, cover, secret):
        ...
        x1 = self.embed_relu1(self.embed_bn1(self.embed_conv1(combined)))
        x2 = self.embed_relu2(self.embed_bn2(self.embed_conv2(x1)))
        x3 = self.embed_relu3(self.embed_bn3(self.embed_conv3(x2)))
        x4 = self.embed_relu4(self.embed_bn4(self.embed_conv4(x3)))
        x5 = self.embed_relu5(self.embed_bn5(self.embed_conv5(x4)))
        x6 = self.embed_relu6(self.embed_bn6(self.embed_conv6(x5)))
        x7 = self.embed_relu7(self.embed_bn7(self.embed_conv7(x6)))
        stego_diff = self.embed_tanh(self.embed_conv8(x7))
    ...
    def extract(self, stego):
        ...
        """
        # Process through extraction network with attention
        x1 = self.extract_relu1(self.extract_bn1(self.extract_conv1(stego)))
        x2 = self.extract_relu2(self.extract_bn2(self.extract_conv2(x1)))
        x2_att = self.extract_attention1(x2)  # Apply attention
        
        x3 = self.extract_relu3(self.extract_bn3(self.extract_conv3(x2_att)))
        x3_att = self.extract_attention2(x3)  # Apply attention
        
        x4 = self.extract_relu4(self.extract_bn4(self.extract_conv4(x3_att)))
        x5 = self.extract_relu5(self.extract_bn5(self.extract_conv5(x4)))
        x5_att = self.extract_attention3(x5)  # Apply attention
        
        x6 = self.extract_relu6(self.extract_bn6(self.extract_conv6(x5_att)))
        x7 = self.extract_relu7(self.extract_bn7(self.extract_conv7(x6)))
        extracted = self.extract_tanh(self.extract_conv8(x7))
...
class EnhancedDiscriminator(nn.Module):
    """
    An enhanced discriminator network with more layers and spectral normalization
...
def enhanced_gan_steganography_loss(cover, stego, secret, recovered_secret, discriminator, 
                                  alpha_hid=1.0, alpha_rec=1.0, alpha_adv=0.02, alpha_perceptual=0.1):
    """
    Enhanced GAN-enhanced loss function for steganography:
    ...
    - Perceptual loss: LPIPS loss to improve visual quality
    """
    # Clamp images to valid range to prevent numerical issues
    ...
    # Perceptual loss for recovery quality
    lpips_fn = calculate_lpips_loss_fn().to(cover.device)
    perceptual_loss = lpips_fn(recovered_secret, secret).mean()
    ...
    # Combined loss with increased weight on recovery
    total_loss = alpha_hid * hiding_loss + alpha_rec * recovery_loss + alpha_adv * adversarial_loss + alpha_perceptual * perceptual_loss

    return total_loss, hiding_loss, recovery_loss, adversarial_loss, perceptual_loss


def train_enhanced_gan_steganography_model(generator, discriminator, dataset, num_epochs=100, 
                                         val_dataset=None, log_dir="runs/enhanced_gan_steganography",
                                         checkpoint_dir="checkpoints", config=None):
    """
    Train the enhanced GAN-enhanced steganography model with enhanced logging and tensorboard visualization.
    """
    print(f"\nTraining Enhanced GAN-Enhanced Steganography model with enhanced logging...")
    ...
    if config:
        ...
        alpha_perceptual = config['training'].get('alpha_perceptual', 0.1)  # New parameter for perceptual loss
        ...
        alpha_perceptual = 0.1  # Weight for perceptual loss
    ...
    stage1_epochs = num_epochs // 2  # First half: focus on hiding
    stage2_epochs = num_epochs - stage1_epochs  # Second half: joint optimization
    ...
    print(f"Loss weights - Hiding: {alpha_hid}, Recovery: {alpha_rec}, Adversarial: {alpha_adv}, Perceptual: {alpha_perceptual}")
    ...
    for epoch in range(num_epochs):
        # Adjust training strategy based on stage
        if epoch < stage1_epochs:
            # Stage 1: Focus on hiding quality
            current_alpha_rec = alpha_rec * 0.1  # Reduce recovery emphasis
            current_alpha_hid = alpha_hid * 2.0  # Increase hiding emphasis
        else:
            # Stage 2: Joint optimization
            current_alpha_rec = alpha_rec
            current_alpha_hid = alpha_hid
        ...
        for cover_tensor, secret_tensor in progress_bar:
            ...
            try:
                ...
                # Compute generator loss (includes all components)
                gen_loss, hiding_loss, recovery_loss, adv_loss, perceptual_loss = enhanced_gan_steganography_loss(
                    cover_tensor, stego_output, secret_tensor, recovered_secret, discriminator,
                    current_alpha_hid, current_alpha_rec, alpha_adv, alpha_perceptual
                )
            ...
            with torch.no_grad():  # Disable gradient computation for validation
                ...
                for val_cover_tensor, val_secret_tensor in val_progress_bar:
                    ...
                    try:
                        ...
                        val_gen_loss, val_hiding_loss, val_recovery_loss, val_adv_loss, val_perceptual_loss = enhanced_gan_steganography_loss(
                            val_cover_tensor, val_stego_output, val_secret_tensor, val_recovered_secret, 
                            discriminator, current_alpha_hid, current_alpha_rec, alpha_adv, alpha_perceptual
                        )
    ...
    print(f"Training completed for Enhanced GAN-Enhanced Steganography model!")
...
def evaluate_model(generator, test_dataset, num_samples=10):
    """
    Comprehensive evaluation of the trained Enhanced GAN-enhanced model.
    """
    print(f"\nEvaluating Enhanced GAN-Enhanced model on {num_samples} test samples...")
...
def create_visualization_grid(results_data, save_path='enhanced_gan_steganography_results.png'):
    """
    Create a comprehensive visualization grid showing results.
    """
    print(f"\nCreating Enhanced GAN visualization grid at: {save_path}")
    ...
    plt.close()
    print(f"Enhanced GAN visualization grid saved at: {save_path}")


def main(config_path="config.json"):
    """
    Main function using config file for Enhanced GAN-enhanced steganography.
    """
    print("Enhanced GAN-Enhanced Steganography Training: Embed Secret in Cover, Extract from Stego")
    print("=" * 90)
    ...
    # Initialize models
    enhanced_gan_steg_model = EnhancedGANSteganographyModel()
    enhanced_discriminator = EnhancedDiscriminator()
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    enhanced_gan_steg_model = enhanced_gan_steg_model.to(device)
    enhanced_discriminator = enhanced_discriminator.to(device)

    print(f"\nEnhanced GAN-Enhanced Steganography Model parameters: {sum(p.numel() for p in enhanced_gan_steg_model.parameters()):,}")
    print(f"Enhanced Discriminator parameters: {sum(p.numel() for p in enhanced_discriminator.parameters()):,}")
    ...
    # Train Enhanced GAN-Enhanced Steganography model with enhanced logging
    print("\nStarting Enhanced GAN-Enhanced Steganography model training (224x224) with enhanced logging...")
    trained_generator, trained_discriminator = train_enhanced_gan_steganography_model(
        enhanced_gan_steg_model,
        enhanced_discriminator,
        train_dataset,
    ...
    print("\n" + "=" * 60)
    print("TRAINING COMPLETED FOR ENHANCED GAN-ENHANCED STEGANOGRAPHY MODEL")
    print("=" * 60)
    ...
    create_visualization_grid(test_results_data, save_path='enhanced_gan_steganography_test_results.png')

    # Save the trained models
    torch.save({
        'generator_state_dict': trained_generator.state_dict(),
        'discriminator_state_dict': trained_discriminator.state_dict(),
        'model_type': 'EnhancedGANSteganographyModel',
        'img_size': img_size,
    }, 'enhanced_gan_steganography_model_50k.pth')

    print(f"\nEnhanced GAN-Enhanced Steganography model saved as 'enhanced_gan_steganography_model_50k.pth'")
